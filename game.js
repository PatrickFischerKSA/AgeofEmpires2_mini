...
// Enthält: Minimap (Pixel), Zivilisationen, Upgrades, KI-Schwierigkeitsgrade
...
const game = {
  ...
  map: {
    width: 16,
    height: 16,
    tiles: [], // flat array of strings: "grass","forest","gold","stone","water","player","enemy"
    playerPos: { x: 3, y: 12 },
    enemyPos: { x: 12, y: 3 }
  }
};

// --- Zivilisations-Labels ---
...
// Ressourcenertrag pro Villager und Sekunde (Basiswerte)
...
// Zeitalter / Eco / Civ / Tech Multiplier pro Ressource
function ecoMultiplierForTask(task) {
  ...
  if (game.civ === "mongols" && task === "food") mul *= 1.20; // Jagd/Food
...
function generateMap() {
  ...
  // Ein paar einfache Patches generieren
...
function renderMinimap() {
  ...
  game.map.tiles.forEach((tileType, index) => {
    const div = document.createElement("div");
    div.classList.add("minimap-tile");
    div.classList.add("tile-" + tileType);
    const x = index % w;
    const y = Math.floor(index / w);
    div.title = `${x}, ${y}`;
    div.addEventListener("click", () => {
      logMessage(`Minimap: Klicke auf Feld (${x}, ${y}) – Typ: ${tileType}.`, "system");
    });
    mm.appendChild(div);
  });
}
...
function updateUI() {
  ...
  // Eco-Raten (grob)
  let rateFood = counts.food * baseRateForTask("food") * ecoMultiplierForTask("food");
  let rateWood = counts.wood * baseRateForTask("wood") * ecoMultiplierForTask("wood");
  let rateGold = counts.gold * baseRateForTask("gold") * ecoMultiplierForTask("gold");
  let rateStone = counts.stone * baseRateForTask("stone") * ecoMultiplierForTask("stone");
  ...
  document.getElementById("ui-base-hp-label").textContent = `${Math.max(0, Math.floor(game.baseHP))} / ${game.baseHPMax}`;
  document.getElementById("ui-enemy-hp-label").textContent = `${Math.max(0, Math.floor(game.enemy.baseHP))} / game.enemy.baseHPMax`.replace("game.enemy.baseHPMax", game.enemy.baseHPMax);
}
...
function enemyAttack() {
  // KI-Eco-Bonus
  const diffEco = game.enemy.ecoBonus;
  let enemyPower = game.enemy.strength * diffEco;

  // Primitive Konterlogik: viele Bogenschützen => mehr Skirm-Faktor
  const totalArmy =
    game.military.inf +
    game.military.spear +
    game.military.skirm +
  ...
  if (totalArmy > 0) {
    ...
    if (archerShare > 0.6) {
      counterBonus += 0.2 * enemyPower; // viele Skirms
    }
    const cavShare = game.military.cav / totalArmy;
    if (cavShare > 0.4) {
      counterBonus += 0.15 * enemyPower; // mehr Speere
    }
  }
...
// ---- Setup (Zivilisation & Schwierigkeit) ----
...
function startGame() {
  ...
  // Start mit 3 Villagern auf Nahrung
...
function handleAction(action) {
  switch (action) {
    ...
    // Eco-Techs
    case "tech-doublebit":
      return buyTech("doubleBit", "Doppelaxt", { food: 100, gold: 50 }, () => game.buildings.lumberCamp || true);
